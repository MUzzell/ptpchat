    ...
    public class RibbonToolTip : ToolTip
    {
        ...
        public RibbonToolTip(IRibbonElement ribbonElement)
        {
            try
            {
                ...
                base.Popup += this.ToolTip_Popup;
                this.Draw += this.ToolTip_Draw;
            }
            catch (Exception ex)
            {
        ...
        private void ToolTip_Draw(object sender, DrawToolTipEventArgs e)
        {
            try
            {
                ...
                var TipText = this.GetToolTip(e.AssociatedControl);

                var eTip = new RibbonToolTipRenderEventArgs(this.Owner, e.Graphics, e.Bounds, TipText);
                eTip.Color = Color.Black;
                eTip.Font = this._Font;

                this.Owner.Renderer.OnRenderToolTipBackground(eTip);
                //e.DrawBackground();
                ...
                if (this._TextRectangle.Width > 0 && this._TextRectangle.Height > 0)
                {
                    ...
                    eTip.ClipRectangle = this._TextRectangle;
                    eTip.Text = TipText;
                    this.Owner.Renderer.OnRenderToolTipText(eTip);
                }

                // Draw the Title
                if (this._TitleRectangle.Width > 0 && this._TitleRectangle.Height > 0)
                {
                    var f = new Font(this._Font, FontStyle.Bold);
                    eTip.ClipRectangle = this._TitleRectangle;
                    eTip.Text = this.ToolTipTitle;
                    eTip.Font = f;
                    this.Owner.Renderer.OnRenderToolTipText(eTip);
                    f.Dispose();
                }
            ...
            try
            {
                if (this.OwnerDraw)
                {
                    if (!this._AutoSize)
                    {
                        ...
                        var CurLeft = this.TipPadding.Left;
                        var CurTop = this.TipPadding.Top;

                        if (!string.IsNullOrEmpty(base.ToolTipTitle))
                        {
                            var fTitle = new Font(this._Font, FontStyle.Bold);
                            sSize = g.MeasureString(base.ToolTipTitle, fTitle, DEFAULT_WIDTH).ToSize();
                            sSize.Width = Math.Max(sSize.Width + 5, DEFAULT_WIDTH);
                            this._TitleRectangle = new Rectangle(CurLeft, CurTop, sSize.Width, sSize.Height);
                            //_TitleRectangle = Rectangle.FromLTRB(CurLeft, CurTop, sSize.Width + CurLeft, CurTop + sSize.Height);
                            CurTop = this._TitleRectangle.Bottom + 5;
                            fTitle.Dispose();
                        }

                        if (this.ToolTipImage != null)
                        {
                            this._ImageRectangle = new Rectangle(CurLeft, CurTop, this.ToolTipImage.Width + 2, this.ToolTipImage.Height + 2);

                            CurLeft = this._ImageRectangle.Right + 10;
                        }
                        ...
                        sSize = g.MeasureString(this.GetToolTip(e.AssociatedControl), this._Font, DEFAULT_WIDTH).ToSize();
                        this._TextRectangle = Rectangle.FromLTRB(CurLeft, CurTop, sSize.Width + CurLeft + 4, CurTop + sSize.Height);
                        CurLeft = this._TextRectangle.Right;

                        //now calculate the total size of the tooltip and set the size
                        var TSize = new Size(
                            Math.Max(this._TextRectangle.Right, this._TitleRectangle.Right) + this.TipPadding.Right,
                            Math.Max(this._TextRectangle.Bottom, this._ImageRectangle.Bottom) + this.TipPadding.Bottom);
                        e.ToolTipSize = TSize;
                    }
                    else
                    {
                        ...
                        {
                            this._ImageWidth = oldSize.Height;
                            oldSize.Width += this._ImageWidth + this.TipPadding.Left;
                        }
                        else
                        {
                            oldSize.Width += this.TipPadding.Left;
                        }
