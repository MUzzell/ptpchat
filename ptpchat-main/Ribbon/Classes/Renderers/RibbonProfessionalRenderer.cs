namespace PtpChat.Main.Ribbon.Classes.Renderers
{
    ...
    using RibbonButtonList = PtpChat.Main.Ribbon.Component_Classes.RibbonButtonList;
    ...
    using RibbonCaptionButton = PtpChat.Main.Ribbon.Component_Classes.RibbonCaptionButton;
    using RibbonComboBox = PtpChat.Main.Ribbon.Component_Classes.RibbonComboBox;
    using RibbonDesigner = PtpChat.Main.Ribbon.Classes.Designers.RibbonDesigner;
    using RibbonDropDown = PtpChat.Main.Ribbon.Component_Classes.RibbonDropDown;
    ...
    using RibbonOrbOptionButton = PtpChat.Main.Ribbon.Component_Classes.RibbonOrbOptionButton;
    ...
    using RibbonProfesionalRendererColorTable = PtpChat.Main.Ribbon.Classes.Renderers.Color_Tables.RibbonProfesionalRendererColorTable;
    ...
    using RibbonToolTipRenderEventArgs = PtpChat.Main.Ribbon.Classes.EventArgs.RibbonToolTipRenderEventArgs;
    using RibbonUpDown = PtpChat.Main.Ribbon.Component_Classes.RibbonUpDown;
    ...
    public class RibbonProfessionalRenderer : RibbonRenderer
    {
        ...
        {
            using (Brush b = new LinearGradientBrush(new Point(r.X, r.Y - 1), new Point(r.Left, r.Bottom), northColor, southColor))
            {
                g.FillRectangle(b, r);
            }
        }
        ...
        public void DrawPressedShadow(Graphics g, Rectangle r)
        {
            ...
            {
                using (var b = new LinearGradientBrush(shadow, Color.FromArgb(50, Color.Black), Color.FromArgb(0, Color.Black), 90))
                {
        ...
        {
            var arrSize = this.arrowSize;

            if (d == RibbonArrowDirection.Left || d == RibbonArrowDirection.Right)
            {
                //Invert size
                arrSize = new Size(this.arrowSize.Height, this.arrowSize.Width);
            }

            var arrowP = new Point(b.Left + (b.Width - arrSize.Width) / 2, b.Top + (b.Height - arrSize.Height) / 2);
            ...
            this.DrawArrow(g, boundsLight, lt, d);
            this.DrawArrow(g, bounds, dk, d);
        }
        ...
        {
            var result = new Rectangle(container.Left + (container.Width - r.Width) / 2, container.Top + (container.Height - r.Height) / 2, r.Width, r.Height);
        ...
        public void DrawCompleteTab(RibbonTabRenderEventArgs e)
        {
            ...
            if (e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2010)
            {
                ...
                using (var path = RoundRectangle(e.Tab.TabContentBounds, 4))
                {
                    ...
                    using (var b = new LinearGradientBrush(new Point(0, e.Tab.TabContentBounds.Top + tabCenter), new Point(0, e.Tab.TabContentBounds.Bottom - 10), north, south))
                    {
                        b.WrapMode = WrapMode.TileFlipXY;
                ...
                using (var path = this.CreateCompleteTabPath_2010(e.Tab))
                {
                ...
                {
                    //Selected glow
                    using (var path = this.CreateTabPath_2010(e.Tab))
                    {
            ...
            else if (e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2013)
            {
                ...
                //Tab border
                using (var path = this.CreateCompleteTabPath_2013(e.Tab))
                {
        ...
        public void DrawTabNormal(RibbonTabRenderEventArgs e)
        {
            ...
            var clip = Rectangle.FromLTRB(e.Tab.TabBounds.Left, e.Tab.TabBounds.Top, e.Tab.TabBounds.Right, e.Tab.TabBounds.Bottom);

            var r = Rectangle.FromLTRB(e.Tab.TabBounds.Left - 1, e.Tab.TabBounds.Top - 1, e.Tab.TabBounds.Right, e.Tab.TabBounds.Bottom);
        ...
        public void DrawTabSelected(RibbonTabRenderEventArgs e)
        {
            ...
            var glossyR = Rectangle.FromLTRB(innerR.Left + 1, innerR.Top + 1, innerR.Right - 1, innerR.Top + e.Tab.TabBounds.Height / 2);
            ...
            if (e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2010)
            {
                ...
                using (var path = RoundRectangle(shadow, 6, Corners.NorthWest | Corners.NorthEast))
                {
                    using (var b = new PathGradientBrush(path))
                    {
                        ...
                        cb.Colors = new[] { Color.Transparent, Color.FromArgb(50, Color.Black), Color.FromArgb(100, Color.Black) };
                ...
                using (var path = RoundRectangle(tab, 6, Corners.North))
                {
                    ...
                    using (var b = new LinearGradientBrush(e.Tab.TabBounds, Theme.ColorTable.TabNorth, Theme.ColorTable.TabSouth, 90))
                    {
                        e.Graphics.FillPath(b, path);
                    }
                }
        ...
        public void DrawPanelNormal(RibbonPanelRenderEventArgs e)
        {
            ...
            {
                var darkBorder = Rectangle.FromLTRB(e.Panel.Bounds.Left, e.Panel.Bounds.Top, e.Panel.Bounds.Right, e.Panel.Bounds.Bottom);

                var lightBorder = Rectangle.FromLTRB(e.Panel.Bounds.Left + 1, e.Panel.Bounds.Top + 1, e.Panel.Bounds.Right + 1, e.Panel.Bounds.Bottom);

                var textArea = Rectangle.FromLTRB(e.Panel.Bounds.Left + 1, e.Panel.ContentBounds.Bottom, e.Panel.Bounds.Right - 1, e.Panel.Bounds.Bottom - 1);
        ...
        public void DrawPanelSelected(RibbonPanelRenderEventArgs e)
        {
            ...
            var lightBorder = Rectangle.FromLTRB(e.Panel.Bounds.Left + 1, e.Panel.Bounds.Top + 1, e.Panel.Bounds.Right - 1, e.Panel.Bounds.Bottom - 1);
            ...
            if (e.Panel.ButtonMoreVisible)
            {
                ...
                {
                    this.DrawButtonPressed(e.Graphics, e.Panel.ButtonMoreBounds, Corners.SouthEast, e.Ribbon);
                }
                else if (e.Panel.ButtonMoreSelected)
                {
                    this.DrawButtonSelected(e.Graphics, e.Panel.ButtonMoreBounds, Corners.SouthEast, e.Ribbon);
                }
        ...
        public void DrawButtonMoreGlyph(Graphics g, Rectangle b, bool enabled)
        {
            ...
            var bounds = this.CenterOn(b, new Rectangle(Point.Empty, this.moreSize));
            var boundsLight = bounds;
            boundsLight.Offset(1, 1);

            this.DrawButtonMoreGlyph(g, boundsLight.Location, light);
            this.DrawButtonMoreGlyph(g, bounds.Location, dark);
        }
        ...
        public void DrawButtonMoreGlyph(Graphics gr, Point p, Color color)
        {
            ...
            var a = p;
            var b = new Point(p.X + this.moreSize.Width - 1, p.Y);
            var c = new Point(p.X, p.Y + this.moreSize.Height - 1);
            var f = new Point(p.X + this.moreSize.Width, p.Y + this.moreSize.Height);
        ...
        public void DrawPannelOveflowSelected(RibbonPanelRenderEventArgs e)
        {
            ...
            using (var b = new LinearGradientBrush(lightBorder, Theme.ColorTable.PanelOverflowBackgroundSelectedNorth, Color.Transparent, 90))
            {
                e.Graphics.FillPath(b, light);
            }
        ...
        public void DrawPanelOverflowPressed(RibbonPanelRenderEventArgs e)
        {
            ...
            var glossy = Rectangle.FromLTRB(e.Panel.Bounds.Left, e.Panel.Bounds.Top, e.Panel.Bounds.Right, e.Panel.Bounds.Top + 17);

            var dark = RoundRectangle(darkBorder, 3);
            var light = RoundRectangle(lightBorder, 3);

            using (var b = new LinearGradientBrush(lightBorder, Theme.ColorTable.PanelOverflowBackgroundPressed, Theme.ColorTable.PanelOverflowBackgroundSelectedSouth, 90))
            {
                b.WrapMode = WrapMode.TileFlipXY;
            ...
            {
                using (var b = new LinearGradientBrush(glossy, Color.FromArgb(150, Color.White), Color.FromArgb(50, Color.White), 90))
                {
                    b.WrapMode = WrapMode.TileFlipXY;
                    e.Graphics.FillPath(b, path);
                }
            ...
            this.DrawPressedShadow(e.Graphics, glossy);
        ...
        public void DrawPanelOverflowImage(RibbonPanelRenderEventArgs e)
        {
            ...
            var imgSquareR = new Rectangle(new Point(e.Panel.Bounds.Left + (e.Panel.Bounds.Width - imgSquareSize.Width) / 2, e.Panel.Bounds.Top + 5), imgSquareSize);

            var imgSquareBottomR = Rectangle.FromLTRB(imgSquareR.Left, imgSquareR.Bottom - 10, imgSquareR.Right, imgSquareR.Bottom);

            var textR = Rectangle.FromLTRB(e.Panel.Bounds.Left + margin, imgSquareR.Bottom + margin, e.Panel.Bounds.Right - margin, e.Panel.Bounds.Bottom - margin);

            using (var imgSq = RoundRectangle(imgSquareR, 5))
            {
                using (var imgSqB = RoundRectangle(imgSquareBottomR, 5, Corners.South))
                {
                    using (var b = new LinearGradientBrush(imgSquareR, Theme.ColorTable.TabContentNorth, Theme.ColorTable.TabContentSouth, 90))
                    {
                    ...
                    if (e.Panel.Image != null)
                    {
                        e.Graphics.DrawImage(
                            ...
                            e.Panel.Image.Width,
                            e.Panel.Image.Height);
                    }

                    if (e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2007 | e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2010)
                    {
                        using (var b = new SolidBrush(this.GetTextColor(e.Panel.Enabled, Theme.ColorTable.Text)))
                        {
                    ...
                    {
                        using (var b = new SolidBrush(this.GetTextColor(e.Panel.Enabled, Theme.ColorTable.RibbonItemText_2013)))
                        {
                            var sf = new StringFormat();
                            sf.Alignment = StringAlignment.Center;
                            sf.LineAlignment = StringAlignment.Near;
                    ...
                    {
                        var bounds = this.LargeButtonDropDownArrowBounds(e.Graphics, e.Panel.Owner.Font, e.Panel.Text, textR);
                        ...
                        if (bounds.Right < e.Panel.Bounds.Right)
                        {
                            ...
                            this.DrawArrow(e.Graphics, boundsLight, lt, RibbonArrowDirection.Down);
                            this.DrawArrow(e.Graphics, bounds, dk, RibbonArrowDirection.Down);
                        }
                    }
                }
            }
        ...
        public void DrawOrbOptionButton(Graphics g, Rectangle bounds)
        {
            ...
            using (var p = RoundRectangle(bounds, 3))
            {
                ...
                this.GradientRect(
                    g,
                    Rectangle.FromLTRB(bounds.Left, bounds.Top + bounds.Height / 2, bounds.Right, bounds.Bottom - 2),
                    Theme.ColorTable.OrbOptionShine,
                    Theme.ColorTable.OrbOptionBackground);
        ...
        public void DrawButton(Graphics g, Rectangle bounds, Corners corners)
        {
            ...
            using (var boundsPath = RoundRectangle(outerR, 3, corners))
            {
                ...
                using (var path = RoundRectangle(glossyR, 3, (corners & Corners.NorthWest) | (corners & Corners.NorthEast)))
                {
                    ...
                    {
                        using (var b = new LinearGradientBrush(glossyR, Theme.ColorTable.ButtonGlossyNorth, Theme.ColorTable.ButtonGlossySouth, 90))
                        {
                            b.WrapMode = WrapMode.TileFlipXY;
                            g.FillPath(b, path);
                        }
        ...
        public Rectangle LargeButtonDropDownArrowBounds(Graphics g, Font font, string text, Rectangle textLayout)
        {
            ...
            if (moreWords)
            {
                return new Rectangle(lastCharBounds.Right + 3, lastCharBounds.Top + (lastCharBounds.Height - this.arrowSize.Height) / 2, this.arrowSize.Width, this.arrowSize.Height);
            }
            return new Rectangle(
                textLayout.Left + (textLayout.Width - this.arrowSize.Width) / 2,
                lastCharBounds.Bottom + (textLayout.Bottom - lastCharBounds.Bottom - this.arrowSize.Height) / 2,
                this.arrowSize.Width,
                this.arrowSize.Height);
        }
        ...
        public void DrawButtonDropDownArrow(Graphics g, RibbonButton button, Rectangle textLayout)
        {
            ...
            {
                bounds = this.LargeButtonDropDownArrowBounds(g, button.Owner.Font, button.Text, textLayout);
            }
            else
            {
            ...
            this.DrawArrowShaded(g, bounds, button.DropDownArrowDirection, button.Enabled);
        }
        ...
        public void DrawButtonDisabled(Graphics g, Rectangle bounds, Corners corners)
        {
            ...
            using (var boundsPath = RoundRectangle(outerR, 3, corners))
            {
                ...
                using (var path = RoundRectangle(glossyR, 3, (corners & Corners.NorthWest) | (corners & Corners.NorthEast)))
                {
                    using (var b = new LinearGradientBrush(glossyR, Theme.ColorTable.ButtonDisabledGlossyNorth, Theme.ColorTable.ButtonDisabledGlossySouth, 90))
                    {
                        b.WrapMode = WrapMode.TileFlipXY;
                        g.FillPath(b, path);
                    }
        ...
        public void DrawButtonPressed(Graphics g, Rectangle bounds, Corners corners, Ribbon ribbon)
        {
            ...
            if (ribbon.OrbStyle == RibbonOrbStyle.Office_2007 | ribbon.OrbStyle == RibbonOrbStyle.Office_2010)
            {
                ...
                using (var boundsPath = RoundRectangle(outerR, 3, corners))
                {
                    ...
                    this.DrawPressedShadow(g, outerR);
                }
            }
        ...
        {
            this.DrawButtonPressed(g, button.Bounds, this.ButtonCorners(button), ribbon);
        }
        ...
        public void DrawButtonChecked(Graphics g, Rectangle bounds, Corners corners)
        {
            ...
            using (var boundsPath = RoundRectangle(outerR, 3, corners))
            {
                ...
                using (var path = RoundRectangle(glossyR, 3, (corners & Corners.NorthWest) | (corners & Corners.NorthEast)))
                {
                    using (var b = new LinearGradientBrush(glossyR, Theme.ColorTable.ButtonCheckedGlossyNorth, Theme.ColorTable.ButtonCheckedGlossySouth, 90))
                    {
                        b.WrapMode = WrapMode.TileFlipXY;
                        g.FillPath(b, path);
                    }
        ...
        {
            this.DrawButtonSelected(g, button.Bounds, this.ButtonCorners(button), ribbon);
        }
        ...
        {
            var outerR = Rectangle.FromLTRB(button.DropDownBounds.Left, button.DropDownBounds.Top, button.DropDownBounds.Right - 1, button.DropDownBounds.Bottom - 1);

            var innerR = Rectangle.FromLTRB(outerR.Left + 1, outerR.Top + 1, outerR.Right - 1, outerR.Bottom - 1);

            var faceOuterR = Rectangle.FromLTRB(button.ButtonFaceBounds.Left, button.ButtonFaceBounds.Top, button.ButtonFaceBounds.Right - 1, button.ButtonFaceBounds.Bottom - 1);
            ...
            var faceCorners = this.ButtonFaceRounding(button);
            var ddCorners = this.ButtonDdRounding(button);
        ...
        public void DrawSplitButtonDropDownSelected(RibbonItemRenderEventArgs e, RibbonButton button)
        {
            ...
            var innerR = Rectangle.FromLTRB(outerR.Left + 1, outerR.Top + (button.SizeMode == RibbonElementSizeMode.Large ? 1 : 0), outerR.Right - 1, outerR.Bottom - 1);
        ...
        {
            var outerR = Rectangle.FromLTRB(grp.Bounds.Left, grp.Bounds.Top, grp.Bounds.Right - 1, grp.Bounds.Bottom - 1);

            var innerR = Rectangle.FromLTRB(outerR.Left + 1, outerR.Top + 1, outerR.Right - 1, outerR.Bottom - 1);

            var glossyR = Rectangle.FromLTRB(outerR.Left + 1, outerR.Top + outerR.Height / 2 + 1, outerR.Right - 1, outerR.Bottom - 1);

            var outer = RoundRectangle(outerR, 2);
            var inner = RoundRectangle(innerR, 2);
            var glossy = RoundRectangle(glossyR, 2);

            using (var b = new LinearGradientBrush(innerR, Theme.ColorTable.ItemGroupBgNorth, Theme.ColorTable.ItemGroupBgSouth, 90))
            {
                e.Graphics.FillPath(b, inner);
            }

            using (var b = new LinearGradientBrush(glossyR, Theme.ColorTable.ItemGroupBgGlossy, Color.Transparent, 90))
            {
                e.Graphics.FillPath(b, glossy);
            }
        ...
        {
            var outerR = Rectangle.FromLTRB(list.Bounds.Left, list.Bounds.Top, list.Bounds.Right - 1, list.Bounds.Bottom);
            ...
            using (var path = RoundRectangle(outerR, 3, Corners.East))
            {
                ...
                {
                    bgcolor = Theme.ColorTable.DropDownBg;
                }
            ...
            else
            {
                ...
                {
                    this.DrawButtonPressed(g, list.ButtonDownBounds, list.ButtonDropDownPresent ? Corners.None : Corners.SouthEast, ribbon);
                }
                else if (list.ButtonDownSelected)
                {
                    this.DrawButtonSelected(g, list.ButtonDownBounds, list.ButtonDropDownPresent ? Corners.None : Corners.SouthEast, ribbon);
                }
                else
                {
                ...
                {
                    this.DrawButtonPressed(g, list.ButtonUpBounds, Corners.NorthEast, ribbon);
                }
                else if (list.ButtonUpSelected)
                {
                    this.DrawButtonSelected(g, list.ButtonUpBounds, Corners.NorthEast, ribbon);
                }
                else
                {
                ...
                if (list.ButtonDropDownPresent)
                {
                    ...
                    {
                        this.DrawButtonPressed(g, list.ButtonDropDownBounds, Corners.SouthEast, ribbon);
                    }
                    else if (list.ButtonDropDownSelected)
                    {
                        this.DrawButtonSelected(g, list.ButtonDropDownBounds, Corners.SouthEast, ribbon);
                    }
                    else
                    {
                ...
                if (list.ScrollType == RibbonButtonList.ListScrollType.Scrollbar && list.ScrollBarEnabled)
                {
                    if (list.ThumbPressed)
                    {
                        this.DrawButtonPressed(g, list.ThumbBounds, Corners.All, ribbon);
                    }
                    else if (list.ThumbSelected)
                    {
                        this.DrawButtonSelected(g, list.ThumbBounds, Corners.All, ribbon);
                    }
                    else
                    {
                ...
                var arrUp = this.CenterOn(list.ButtonUpBounds, new Rectangle(Point.Empty, this.arrowSize));
                arrUp.Offset(0, 1);
                var arrD = this.CenterOn(list.ButtonDownBounds, new Rectangle(Point.Empty, this.arrowSize));
                arrD.Offset(0, 1);
                var arrdd = this.CenterOn(list.ButtonDropDownBounds, new Rectangle(Point.Empty, this.arrowSize));
                arrdd.Offset(0, 3);

                this.DrawArrow(g, arrUp, list.ButtonUpEnabled ? lt : Color.Transparent, RibbonArrowDirection.Up);
                arrUp.Offset(0, -1);
                this.DrawArrow(g, arrUp, list.ButtonUpEnabled ? dk : ds, RibbonArrowDirection.Up);

                this.DrawArrow(g, arrD, list.ButtonDownEnabled ? lt : Color.Transparent, RibbonArrowDirection.Down);
                arrD.Offset(0, -1);
                this.DrawArrow(g, arrD, list.ButtonDownEnabled ? dk : ds, RibbonArrowDirection.Down);
                ...
                if (list.ButtonDropDownPresent)
                {
                    using (var b = new SolidBrush(Theme.ColorTable.Arrow))
                    {
                        var sm = g.SmoothingMode;
                        g.SmoothingMode = SmoothingMode.None;
                        g.FillRectangle(b, new Rectangle(new Point(arrdd.Left - 1, arrdd.Top - 4), new Size(this.arrowSize.Width + 2, 1)));
                        g.SmoothingMode = sm;
                    }

                    this.DrawArrow(g, arrdd, lt, RibbonArrowDirection.Down);
                    arrdd.Offset(0, -1);
                    this.DrawArrow(g, arrdd, dk, RibbonArrowDirection.Down);
                }
        ...
        public void DrawComboxDropDown(Graphics g, RibbonComboBox b, Ribbon ribbon)
        {
            ...
            {
                this.DrawButtonPressed(g, b.DropDownButtonBounds, Corners.None, ribbon);
            }
            else if (b.DropDownButtonSelected)
            {
                this.DrawButtonSelected(g, b.DropDownButtonBounds, Corners.None, ribbon);
            }
            else if (b.Selected)
            {
            ...
            this.DrawArrowShaded(g, b.DropDownButtonBounds, RibbonArrowDirection.Down, true); //b.Enabled);
        }
        ...
        public void DrawUpDownButtons(Graphics g, RibbonUpDown b, Ribbon ribbon)
        {
            ...
            {
                this.DrawButtonPressed(g, b.UpButtonBounds, Corners.None, ribbon);
            }
            else if (b.UpButtonSelected)
            {
                this.DrawButtonSelected(g, b.UpButtonBounds, Corners.None, ribbon);
            }
            else
            {
                this.DrawButton(g, b.UpButtonBounds, Corners.None);
            }

            if (b.DownButtonPressed)
            {
                this.DrawButtonPressed(g, b.DownButtonBounds, Corners.None, ribbon);
            }
            else if (b.DownButtonSelected)
            {
                this.DrawButtonSelected(g, b.DownButtonBounds, Corners.None, ribbon);
            }
            else
            {
                this.DrawButton(g, b.DownButtonBounds, Corners.None);
            }
        ...
        public void DrawCaptionBarBackground(Rectangle r, Graphics g)
        {
            ...
            Color[,] colors =
                {
                    { Theme.ColorTable.Caption1, Theme.ColorTable.Caption2 }, { Theme.ColorTable.Caption3, Theme.ColorTable.Caption4 },
                    { Theme.ColorTable.Caption5, Theme.ColorTable.Caption6 }, { Theme.ColorTable.Caption7, Theme.ColorTable.Caption7 }
                };
        ...
        public void DrawOrb(Graphics g, Rectangle r, Image image, bool selected, bool pressed)
        {
            ...
            using (var p = new GraphicsPath())
            {
                ...
                using (var b = new LinearGradientBrush(new Point(rinner.Left, rinner.Bottom), new Point(rinner.Left, pt.Y - 1), light, Color.FromArgb(50, light)))
                {
                    g.FillPath(b, p);
                }
        ...
        public override void OnRenderOrbDropDownBackground(RibbonOrbDropDownEventArgs e)
        {
            ...
            var southSouthRect = Rectangle.FromLTRB(1, (Height - OrbDDContent.Bottom) / 2 + OrbDDContent.Bottom, Width - 1, Height - 1);
            ...
            this.GradientRect(e.Graphics, NorthNorthRect, OrbDropDownNorthA, OrbDropDownNorthB);
            this.GradientRect(e.Graphics, northSouthRect, OrbDropDownNorthC, OrbDropDownNorthD);
            this.GradientRect(e.Graphics, southSouthRect, OrbDropDownSouthC, OrbDropDownSouthD);
            ...
            if (e.RibbonOrbDropDown.ContentRecentItemsCaptionBounds.Height > 0)
            {
                ...
                {
                    e.Graphics.DrawLine(p, new Point(OrbDDContent.Left, cb.Bottom - linePos), new Point(OrbDDContent.Right, cb.Bottom - linePos));
                }
                using (var p = new Pen(OrbDropDownSeparatordark))
                {
                    e.Graphics.DrawLine(p, new Point(OrbDDContent.Left, cb.Bottom - linePos - 1), new Point(OrbDDContent.Right, cb.Bottom - linePos - 1));
                }
            ...
            if (e.Ribbon.OrbVisible && e.Ribbon.CaptionBarVisible && e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2007)
            {
                ...
                this.DrawOrb(e.Graphics, orbb, e.Ribbon.OrbImage, e.Ribbon.OrbSelected, e.Ribbon.OrbPressed);
            }
        ...
        public override void OnRenderRibbonQuickAccessToolbarBackground(RibbonRenderEventArgs e)
        {
            ...
            using (var path = this.CreateQuickAccessPath(a, b, c, d, z, bounds, 0, 0, e.Ribbon))
            {
                if (!aero)
                {
                ...
                if (!aero)
                {
                    using (var br = new LinearGradientBrush(b, d, Color.FromArgb(150, Theme.ColorTable.QuickAccessUpper), Color.FromArgb(150, Theme.ColorTable.QuickAccessLower)))
                    {
        ...
        public override void OnRenderRibbonOrb(RibbonRenderEventArgs e)
        {
            if (e.Ribbon.OrbVisible)
            {
                ...
                else if (e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2010 | e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2013) //Michael Spradlin - 05/03/2013 Office 2013 Style Changes
                {
                    ...
                    {
                        this.DrawOrbPressed(args);
                    }
                    else if (e.Ribbon.OrbSelected)
                    {
                        this.DrawOrbSelected(args);
                    }
                    else
                    {
                        this.DrawOrbNormal(args);
                    }
        ...
        public override void OnRenderRibbonTabText(RibbonTabRenderEventArgs e)
        {
            ...
            if (e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2007 | e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2010) //Michael Spradlin - 05/03/2013 Office 2013 Style Changes
            {
                using (Brush b = new SolidBrush(this.GetTextColor(true, e.Tab.Active ? Theme.ColorTable.TabActiveText : Theme.ColorTable.TabText)))
                {
            ...
            {
                using (Brush b = new SolidBrush(this.GetTextColor(true, e.Tab.Active | e.Tab.Selected ? Theme.ColorTable.TabText_2013 : Theme.ColorTable.TabTextSelected_2013)))
                {
                    e.Graphics.TextRenderingHint = TextRenderingHint.ClearTypeGridFit;
                    e.Graphics.DrawString(e.Tab.Text, e.Ribbon.RibbonTabFont, b, r, sf);
                }
        ...
        public override void OnRenderRibbonPanelText(RibbonPanelRenderEventArgs e)
        {
            ...
            if (e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2007 | e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2010)
            {
                using (Brush b = new SolidBrush(this.GetTextColor(e.Panel.Enabled, Theme.ColorTable.PanelText)))
                {
            ...
            {
                using (Brush b = new SolidBrush(this.GetTextColor(e.Panel.Enabled, Theme.ColorTable.PanelText_2013)))
                {
                    e.Graphics.DrawString(e.Panel.Text, e.Ribbon.Font, b, textArea, sf);
                }
            }
        ...
        public override void OnRenderRibbonItem(RibbonItemRenderEventArgs e)
        {
            if (e.Item is RibbonButton)
            {
                ...
                if (b.Enabled)
                {
                    if (b.Style == RibbonButtonStyle.Normal)
                    {
                        ...
                        {
                            this.DrawButtonPressed(e.Graphics, b, e.Ribbon);
                        }
                        else if (b.Selected)
                        {
                            if (b.Checked)
                            ...
                            {
                                this.DrawButtonSelected(e.Graphics, b, e.Ribbon);
                            }
                        }
                        else if (b.Checked)
                        {
                            this.DrawButtonChecked(e.Graphics, b);
                        }
                        else if (b is RibbonOrbOptionButton)
                        {
                            this.DrawOrbOptionButton(e.Graphics, b.Bounds);
                        }
                    }
                    else
                    {
                        ...
                        {
                            this.DrawButtonPressed(e.Graphics, b, e.Ribbon);
                            this.DrawSplitButtonDropDownSelected(e, b);
                        }
                        else if (b.Pressed && b.SizeMode != RibbonElementSizeMode.DropDown)
                        {
                            this.DrawButtonPressed(e.Graphics, b, e.Ribbon);
                            this.DrawSplitButtonSelected(e, b);
                        }
                ...
                if (b.Style != RibbonButtonStyle.Normal && !(b.Style == RibbonButtonStyle.DropDown && b.SizeMode == RibbonElementSizeMode.Large))
                {
                    ...
                    {
                        this.DrawButtonDropDownArrow(e.Graphics, b, b.OnGetDropDownBounds(b.SizeMode, b.Bounds));
                    }
                    else
                    {
                        this.DrawButtonDropDownArrow(e.Graphics, b, b.DropDownBounds);
                    }
                }
            ...
            else if (e.Item is RibbonUpDown)
            {
                ...
                if (t.Enabled)
                {
                    ...
                    {
                        this.DrawTextBoxSelected(e.Graphics, t.TextBoxBounds);
                    }
                    else
                    {
                        this.DrawTextBoxUnselected(e.Graphics, t.TextBoxBounds);
                    }
                }
                else
                {
                    this.DrawTextBoxDisabled(e.Graphics, t.TextBoxBounds);
                }

                this.DrawUpDownButtons(e.Graphics, t, e.Ribbon);
            ...
            else if (e.Item is RibbonTextBox)
            {
                ...
                {
                    this.DrawComboxDropDown(e.Graphics, t as RibbonComboBox, e.Ribbon);
                }
        ...
        public override void OnRenderRibbonItemText(RibbonTextEventArgs e)
        {
            ...
            if (e.Item is RibbonButton)
            {
                ...
                if (b is RibbonCaptionButton)
                {
                    if (WinApi.IsWindows)
                    {
                        f = new Font(RibbonCaptionButton.WindowsIconsFont, f.Size);
                    }
                    embedded = true;
                ...
                {
                    this.DrawButtonDropDownArrow(e.Graphics, b, e.Bounds);
                }
            ...
            else if (e.Item is RibbonSeparator)
            {
                if (e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2007 | e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2010)
                {
                    foreColor = this.GetTextColor(e.Item.Enabled, Theme.ColorTable.Text);
                }
                else if (e.Ribbon.OrbStyle == RibbonOrbStyle.Office_2013)
                {
                    foreColor = this.GetTextColor(e.Item.Enabled, Theme.ColorTable.RibbonItemText_2013);
                }
            }
        ...
        public override void OnRenderPanelPopupBackground(RibbonCanvasEventArgs e)
        {
            ...
            var darkBorder = Rectangle.FromLTRB(e.Bounds.Left, e.Bounds.Top, e.Bounds.Right, e.Bounds.Bottom);

            var lightBorder = Rectangle.FromLTRB(e.Bounds.Left + 1, e.Bounds.Top + 1, e.Bounds.Right - 1, e.Bounds.Bottom - 1);

            var textArea = Rectangle.FromLTRB(e.Bounds.Left + 1, pnl.ContentBounds.Bottom, e.Bounds.Right - 1, e.Bounds.Bottom - 1);
        ...
        public override void OnRenderDropDownBackground(RibbonCanvasEventArgs e)
        {
            ...
            {
                var gripArea = Rectangle.FromLTRB(e.Bounds.Left + 1, e.Bounds.Bottom - dd.SizingGripHeight, e.Bounds.Right - 1, e.Bounds.Bottom - 1);

                using (var b = new LinearGradientBrush(gripArea, Theme.ColorTable.DropDownGripNorth, Theme.ColorTable.DropDownGripSouth, 90))
                {
                ...
                this.DrawGripDot(e.Graphics, new Point(gripArea.Right - 7, gripArea.Bottom - 3));
                this.DrawGripDot(e.Graphics, new Point(gripArea.Right - 3, gripArea.Bottom - 7));
                this.DrawGripDot(e.Graphics, new Point(gripArea.Right - 3, gripArea.Bottom - 3));
            }
        }
        ...
        public override void OnRenderTabScrollButtons(RibbonTabRenderEventArgs e)
        {
            if (e.Tab.ScrollLeftVisible)
            {
                ...
                {
                    this.DrawButtonSelected(e.Graphics, e.Tab.ScrollLeftBounds, Corners.West, e.Ribbon);
                }
                else
                {
            ...
            if (e.Tab.ScrollRightVisible)
            {
                ...
                {
                    this.DrawButtonSelected(e.Graphics, e.Tab.ScrollRightBounds, Corners.East, e.Ribbon);
                }
                else
                {
        ...
        public override void OnRenderScrollbar(Graphics g, Control ctl, Ribbon ribbon)
        {
            ...
            else
            {
                ...
                if (!list.ButtonDownEnabled)
                {
                    this.DrawButtonDisabled(g, list.ButtonDownBounds, Corners.SouthEast);
                }
                else if (list.ButtonDownPressed)
                {
                    this.DrawButtonPressed(g, list.ButtonDownBounds, Corners.SouthEast, ribbon);
                }
                else if (list.ButtonDownSelected)
                {
                    this.DrawButtonSelected(g, list.ButtonDownBounds, Corners.SouthEast, ribbon);
                }
                else
                {
                    this.DrawButton(g, list.ButtonDownBounds, Corners.None);
        ...
        {
            var darkBorder = Rectangle.FromLTRB(e.ClipRectangle.Left, e.ClipRectangle.Top, e.ClipRectangle.Right - 1, e.ClipRectangle.Bottom - 1);

            var lightBorder = Rectangle.FromLTRB(e.ClipRectangle.Left + 1, e.ClipRectangle.Top + 1, e.ClipRectangle.Right - 2, e.ClipRectangle.Bottom - 1);
            ...
            //   e.Graphics.FillPath(b, dark);
            //}
            using (var b = new LinearGradientBrush(e.ClipRectangle, Theme.ColorTable.ToolTipContentNorth, Theme.ColorTable.ToolTipContentSouth, 90))
            {
                e.Graphics.FillPath(b, dark);
            }
